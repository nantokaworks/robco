...
//! `Command`: `Tasks`, `Log`, `Diff`, `Help`. Kept apart from the agent and
//! coordination tools for the same reason `git_ops` is: a distinct source
//! (the Discord ledger/decision log) rather than the agent registry.
//!
//! Every other Discord-only command (`Skip`, `Retry`, `TaskCreate`, `Kill`,
//! `Panic`) stays Discord-only in this pass — see the `Command::mcp_tool_name`
//! doc comment in `overseer::discord::mcp_bridge` for why.
