...
use crate::{
    ...
    model::Status,
    overseer::{OVERSEER_AGENT_ID, inbox::InboxReport, is_overseer_child},
    registry::Registry,
    status::StatusReport,
    tmux,
};
...
fn resolve_identities(
    ...
    let target_agent_id = match explicit_target {
        ...
        // No explicit target and no session already naming its parent: the
        // only other place a lifecycle report can go is the Overseer inbox
        // (`is_overseer_child` downstream matches this constant), so a
        // worker no longer needs `ROBCO_PARENT_AGENT_ID` set just to repeat
        // a string the binary already knows. A session that does set it
        // keeps going through that same path unchanged.
        None => {
            non_empty(lookup(ENV_PARENT_AGENT_ID)).unwrap_or_else(|| OVERSEER_AGENT_ID.to_string())
        }
    };
    Ok(ReportIdentities {
        target_agent_id,
