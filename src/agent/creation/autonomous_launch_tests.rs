//! dropr:538 — proof that the two TUI creation paths (`src/ui/input.rs` and
//! `src/ui/actions/dropr_task_worker.rs`, both via `create_agent`) resolve
//! the profile's `autonomous_args` and that the env blocklist cannot come
//! apart from the permission flag. Split from `creation/tests.rs` to keep
...
#[test]
fn autonomous_launch_applies_the_blocklist_when_the_profile_has_args() {
    ...
    let (args, env) = autonomous_launch(&config);
...
#[test]
fn autonomous_launch_skips_the_blocklist_when_the_profile_has_no_args() {
    let mut config = Config {
        default_program: "claude".into(),
        profiles: vec![crate::config::Profile {
            name: "claude".into(),
...
#[test]
fn autonomous_launch_with_no_matching_profile_is_empty_not_an_error() {
    let config = Config {
        default_program: "claude".into(),
        profiles: Vec::new(),
        ..Config::default()
