...
fn footer_carries_only_the_essential_hints() {
    let line = hints_line(None, None, None).to_string();
    assert_eq!(line, "[↵] ATTACH [n] NEW [m] MERGE [?] HELP [q] QUIT");
}

#[test]
fn agent_row_advertises_its_own_actions() {
    let line = hints_line(None, Some(Selection::Agent { repo: 0, agent: 0 }), None).to_string();
    assert_eq!(
        line,
...
fn repo_row_advertises_its_own_actions() {
    let line = hints_line(None, Some(Selection::Repo(0)), None).to_string();
    assert_eq!(
        line,
...
fn overseer_ai_row_advertises_attach_and_instruct() {
    let line = hints_line(None, Some(Selection::OverseerAi), None).to_string();
...
fn inbox_item_advertises_answer_approve_dismiss_clear() {
    let line = hints_line(None, Some(Selection::OverseerInbox(0)), None).to_string();
    assert_eq!(
        line,
...
#[test]
fn dropr_task_body_focus_advertises_start_and_back() {
    let line = hints_line(
        None,
        Some(Selection::Repo(0)),
...
fn discord_channel_advertises_retry_and_remove() {
    let line = hints_line(None, Some(Selection::DiscordChannel(0)), None).to_string();
    assert_eq!(line, "[↵] ATTACH [r] RETRY [x] REMOVE [?] HELP [q] QUIT");
}

#[test]
fn orphan_advertises_attach_and_remove() {
    let line = hints_line(None, Some(Selection::Orphan(0)), None).to_string();
    assert_eq!(line, "[↵] ATTACH [x] REMOVE [?] HELP [q] QUIT");
}

#[test]
fn section_headers_carry_no_extra_action() {
    let line = hints_line(None, Some(Selection::OtherHeader), None).to_string();
    assert_eq!(line, "[?] HELP [q] QUIT");
    let line = hints_line(None, Some(Selection::OrphanHeader), None).to_string();
    assert_eq!(line, "[?] HELP [q] QUIT");
}
