...
pub(crate) fn lines(locale: Locale) -> Vec<Line<'static>> {
    ...
    vec![
        ...
        Line::from(""),
        l("Worker escalations"),
        l("  y              on a worker row: approve the newest actionable escalation"),
        l("                 (sends y + enter)"),
        l("  d              on a worker row: dismiss its newest escalation"),
        l("                 (hides the escalation only)"),
