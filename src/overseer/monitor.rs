...
///
/// A worker can lose Overseer ownership — its `parent_agent_id` changes, or
/// the entry survives from a registry written before the worker was
/// enrolled — and without this the entry would sit in the ledger frozen
/// forever: never advanced, never cleaned up, and still occupying a dispatch
/// slot for a worker Overseer may no longer touch. Dropping the row is what
/// keeps the ledger and ownership from disagreeing — an unowned worktree is
/// now exactly a hand-made one, which Overseer never tracked in the first
/// place.
///
...
/// `reconcile_entry`, which would otherwise ask `overseer::release_pipeline`
/// to reconsider the same pull request on every pass its worktree removal
