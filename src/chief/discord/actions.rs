...
fn execute(
    ...
    match command {
        ...
        Command::Dispatch(enabled) => {
            command::set_runtime(ChiefSetting::Dispatch, *enabled)?;
            Ok(format!("dispatch: {}", on_off(*enabled)))
        }
        Command::AutoMerge(false) => {
            command::set_runtime(ChiefSetting::AutoMerge, false)?;
...
fn status() -> crate::Result<String> {
    ...
    let active = ledger
        ...
        .count();
    Ok(format!(
        "chief={} dispatch={} automerge={} workers={}/{} today={}/{}",
