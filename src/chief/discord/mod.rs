...
fn supervisor(
    ...
    while !stop.load(Ordering::Acquire) {
        ...
        match result {
            Ok(Err(error)) => eprintln!("chief: Discord bot stopped: {error}"),
            Ok(Ok(())) => eprintln!("chief: Discord gateway closed; reconnecting"),
            Err(_) => eprintln!("chief: Discord bot panicked; restarting"),
        }
