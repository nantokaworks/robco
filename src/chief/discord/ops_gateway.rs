            ...
            Effect::OpenThread(case) => {
                ...
                match reconcile_thread(http, parent, &case).await {
                    Ok(Some(thread_id)) => match ops.thread_opened(case, thread_id) {
                        ...
                        Err(error) => eprintln!("chief: Discord thread mapping failed: {error}"),
                    },
                    Ok(None) => {}
                    Err(error) => eprintln!("chief: Discord thread reconciliation failed: {error}"),
                }
            }
            Effect::Opening {
                case_id,
                ...
                if delivered && let Err(error) = ops.opening_delivered(&case_id) {
                    eprintln!("chief: Discord opening marker failed: {error}");
                }
            }
                ...
                let archived = !archive
                    || posted
                        && match http.update_thread(channel).archived(true).await {
                            Ok(_) => true,
                            Err(error) => {
                                eprintln!("chief: Discord thread archive failed: {error}");
                                false
                            }
                        };
                        ...
                        match audit_delivery_exhausted(&case_id, posted, archived) {
                            Ok(()) => {
                                if let Err(error) = ops.finalize_exhausted_resolution(&case_id) {
                                    eprintln!("chief: exhaustion marker failed: {error}");
                                }
                            }
                            Err(error) => eprintln!(
                                "chief: failed to audit resolution delivery exhaustion: {error}"
                            ),
                        }
                    }
                    Ok(_) => {}
                    Err(error) => eprintln!("chief: resolution delivery marker failed: {error}"),
                }
            }
        }
    }
...
fn audit_refusal(user_id: &str, reason: &str) {
    ...
    if let Err(error) = logging::append(&entry) {
        eprintln!("chief: failed to audit Discord refusal: {error}");
    }
}
