...
pub fn install() -> Result<Action> {
    ...
    let after = upsert_claude(before.as_deref())?;
...
pub fn uninstall() -> Result<Action> {
    ...
    let Some(after) = remove_claude(&content)? else {
        return Ok(Action::NotPresent);
    };
    write_config(&path, &after)?;
    Ok(Action::Removed)
...
pub fn upsert_claude(content: Option<&str>) -> Result<String> {
...
pub fn remove_claude(content: &str) -> Result<Option<String>> {
...
fn config_path() -> Result<PathBuf> {
    Ok(super::home_dir()?.join(".claude.json"))
}
