...
remote host, per repo, per agent's child worktrees, the `OTHER LOCATIONS` and `ORPHAN SESSIONS` sections,
...
| `←` / `→` (or `h` / `l`) | Collapse / expand a host, repo, an agent's slot-worktree subtree, a section, or OVERSEER Discord |
| `Enter` | Toggle a host/section header, or attach to the selected agent, Control AI, Discord channel, or shell session |
...
Each remote host is a top-level collapsible PROJECTS row. Its connection glyph and summary
stay on that row, while its repositories, Control AI, and Discord channels nest below it. Attaching
...
Each connected host also lists its `Control AI` row and retained Discord channel
rows after that host's repositories. Enter attaches the corresponding remote session;
missing sessions are reported instead of attached. `i` instructs the remote control AI.
A red `⚠` on a connected host row means its
Overseer daemon is not alive.
A failed host has a selectable error child, whose INFO pane shows the full connection error.
