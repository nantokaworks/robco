...
Every repo and agent row carries an accent-coloured, single-column management marker
...
an attribute of that row instead of a fixed row-head column shared with the repo above it. A
deeper row carries the marker further right.

By default the marker is round: `●` for Overseer Auto, `○` for Overseer Manual, blank for a
worktree the Overseer does not own. With `project_icon = "nerdfont"` (see
[09-config-reference.md](09-config-reference.md#project_icon)) the marker set switches to a
Nerd Font bolt/hand pictograph pair instead — a stronger visual contrast than filled-vs-hollow
circles, available only once a patched font is known to be present. `project_icon` also
controls the PROJECTS fold glyph; reusing the same field means one setting governs every
Nerd Font glyph in the tree rather than two independent toggles.

A repo row always shows its own marker, unconditionally. An agent row's marker blanks only
when it is Manual **and** its repo is also Manual — the repo row already said as much, so
that one case is left blank to avoid a marker on every row reading as wallpaper. An Auto
agent row is **always** shown, even when its repo is also Auto (the common case): blanking it
there used to render it identically to an unmanaged worktree, which is the ambiguity this
marker exists to prevent. The marker spends a cell the row prefix already reserved, so titles
start at the same column whether or not a row carries a visible marker, at every depth.
...
and the OVERSEER Inbox — plus the PROJECTS order is persisted to
...
Of the four OVERSEER categories only **Inbox** expands, because its items are rows the
operator acts on. Health, Ledger, and Decisions are single rows: their detail is read-only
and the Info preview pane already shows it in full, so an arrow there would spend a nesting
level on duplicated content. All four labels still start at the same column — the arrow
cell is reserved on every category row and left blank on the three that have no children.
...
| `←` / `→` (or `h` / `l`) | Collapse / expand a repo, an agent's slot-worktree subtree, or the OVERSEER Inbox |
