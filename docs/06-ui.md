...
An agent row the Overseer dispatches to automatically carries an accent-coloured `▶` at the
**row head**, between the selection cursor and the identity-tree indent. It is state, not
activity, so it sits in a fixed column that lines up down the whole tree instead of trailing
the title where every row would put it somewhere else.
...
start at the same column whether or not a row is marked.

Because that cell would otherwise have read as indentation, every agent row carries a
nesting step of its own — one applied left of the identity-tree indent, so an agent title
always starts right of its repo name and a marked row reads as a child of the repo above it
rather than a sibling. The step is one nesting level and no more: the PROJECTS pane is
narrow, and each column spent here is a column taken from the agent title. See
